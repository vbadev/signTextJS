    ...
    if (cert.certType & Ci.nsIX509Cert.USER_CERT) {
      ...
      if (now >= cert.validity.notBefore / 1000 &&
        ...
        if (!caNames) {
          ...
          for (let name of caNames) {
            if (name === cert.issuerName) {
              userCerts.push(cert);
            }
          }
        }
...
function selectCert(userCerts, text) {
  ...
  sandboxDeclarations += "var selected;\n";
  ...
  for (let cert of userCerts) {
    ...
    sandbox.certs[cert.nickname].selected = cert.nickname === gLastSelectedCert;
  }
  ...
  if (sandbox.cancelled) {
    ...
    gLastSelectedCert = sandbox.selectedCert;
  }
  return result;
}
...
function signText(domWindow, text) {
  ...
  if (arguments.length > 3) {
    caNames = Array.prototype.slice.call(arguments, 3);
  }
  let userCerts = getUserCerts(caNames);
  ...
  try {
    ...
    cert = CERT_FindUserCertByUsage(certDB, certName.nickname,
                                    certUsageEmailSigner, true, null);
    if (cert.isNull()) {
    ...
    while (it && !it.isNull()) {
      if (PK11_NeedLogin(it.contents.slot)) {
        cLogin++;
        let rv = PK11_CheckUserPassword(it.contents.slot, certName.password);
        if (rv != SECSuccess) {
    ...
    if (status != SECSuccess || (cLogin > 0 && cLogin == cBadPwd)) {
      log("Authentication for '" + certName.nickname + "' failed");
      cleanupSignTextResources(cert, contentInfo, encodedItem, slotList);
